            ...
            [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        return "<div href='" + row.id + "' class='m_c_e-btn-edit' style='text-decoration:underline;cursor:pointer' mce_loc='/document/acceptance_document' title='Редактировать'>" + row.documentNumber + "</div>";
                    },
                    width: '400px'
                },
                {
...
function create_s_datatable (id) {
    ...
    $("#s_datatable").DataTable({
        ...
        initComplete: function () {
            $(`<button class="btn buttons-create btn-outline-secondary m_c_e-btn-create ml-3" mce_loc="/document/acceptance_document_item">Добавить</button>`)
...
$(document)
    .on('edit create', '.m_c_e-form[mce_loc="/document/acceptance_document"]', function (e, res) {
        res = JSON.parse(res);
        create_s_datatable(res.id);
        $('#s_datatable').css('width', '100%');
    })
    .on('creating', '.m_c_e-form[mce_loc="/document/acceptance_document_item"]', function (e, res) {
        $('.m_c_e-modal[mce_loc="/document/acceptance_document"]').modal("hide");
        $('.m_c_e-form[mce_loc="/document/acceptance_document_item"] #acceptanceDocument').val($('.m_c_e-form[mce_loc="/document/acceptance_document"] #id').val());
    })
    .on('hidden.bs.modal', '.m_c_e-modal[mce_loc="/document/acceptance_document_item"]', function (e) {
        $('.m_c_e-modal[mce_loc="/document/acceptance_document"]').modal("show");
        s_datatable_reload();
    })
    .on('creating', '.m_c_e-form[mce_loc="/document/acceptance_document"]', function (e, res) {
        res = JSON.parse(res);
        $('.m_c_e-form[mce_loc="/document/acceptance_document"]')
